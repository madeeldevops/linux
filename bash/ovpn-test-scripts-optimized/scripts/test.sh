...
#    		    fi
		    ...
		    if ping -c "$PING_COUNT" -W "$PING_TIMEOUT" "$PING_TARGET" >/dev/null 2>&1; then
    			PASSED_MAP["$IP"]+="$PORT "
			echo "$CONFIG $IP $PORT SUCCESS" >> new_results.log
    			CONNECTED=1
    			break
		    else
			...
			echo "$CONFIG $IP $PORT NO_INTERNET" >> new_results.log
    			CONNECTED=2
    			break
		    fi
		fi
	...
	# If never connected then CONNECTED will be 0
        if [[ $CONNECTED -eq 0 ]]; then
            ...
            FAILED_MAP["$IP"]+="$PORT "
	    echo "$CONFIG $IP $PORT FAILED" >> new_results.log
...
for CONFIG in *.ovpn; do
    # HAS_OUTPUT=0 (disabled)

    # Check if this config has failures or no-internet entries (disabled)
...
#    done
    ...
    # Skip clean configs entirely (disabled)
