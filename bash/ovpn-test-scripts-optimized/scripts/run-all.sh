...
exec 9>"$LOCKFILE" # creates/truncates the lock file
if ! flock -n 9; then
...
# Make new_results_merged.log readable (OPTIONAL, must enable with "split-ovpn-remotes.sh", otherwise results will be messy)
"$SCRIPT_DIR/pretty-results.sh" # will output ovpn_testing_report.txt in logs dir
...
echo "⏳ Total runtime for full script execution: $RUNTIME"
